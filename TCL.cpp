...
vector<string> getSelectionArray(int sxi){
    int i,j;
    vector<string> result;
    for(i=0;i<red_query_process_count();i++){
        result.push_back("-");
    }
    for(i=0;i<red_query_sync_xtion_party_count(RED_USE_DECLARED_SYNC_XTION,sxi);i++){
        int indexProcess;
        indexProcess=red_query_sync_xtion_party_process(RED_USE_DECLARED_SYNC_XTION,sxi,i);
        int indexXtion;
        indexXtion=red_query_sync_xtion_party_xtion(RED_USE_DECLARED_SYNC_XTION,sxi,i);
        string stringSelection;
        stringSelection="";
        for(j=0;j<red_query_xtion_attribute(indexXtion, RED_XTION_SYNC_COUNT);j++){  
            stringSelection=stringSelection+red_query_string_xtion_sync(indexXtion,j);
        }
        if(stringSelection!=""){
            result[indexProcess-1]=stringSelection;
        }
    }
    return result;
}
    ...
    for(i=1;i<sxiCount;i++){
        ...
        if(red_and(temp_red,red_query_diagram_enhanced_global_invariance())!=red_false()){
            if(node_map[temp_string.assign(red_diagram_string(temp_red))]==0){
                ...
                temp_edge->selectionArray=getSelectionArray(i);
                temp_edge->src=root;
...
void labelPlayerSelectionOnEdge(TCL_Game_Node* root){
    ...
    selectionName.push_back("bank_choose");
    selectionName.push_back("partner_choose");
    selectionName.push_back("client_choose");
    char temp[20];
    int i,j,k;
